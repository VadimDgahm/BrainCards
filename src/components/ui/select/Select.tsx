...
export const Select = forwardRef<ElementRef<typeof SelectRadix.Root>, SelectorPropsType>(
  ({ options, title, variant = 'default', ...rest }, ref) => {
    const optionsItem = options.map((op, i) => (
      <SelectRadix.Item className={s.SelectItem} key={i} value={op.value}>
        <SelectRadix.ItemText>{op.title}</SelectRadix.ItemText>
      </SelectRadix.Item>
    ))
        ...
        <SelectRadix.Root {...rest}>
          <div className={s.back}>
            <SelectRadix.Trigger
              ...
              ref={ref}
            >
              <SelectRadix.Value placeholder={variant === 'pagination' ? options[0].title : ''} />
              <SelectRadix.Icon>
